...
import { GameState, LeaderboardEntry, Difficulty } from './types';
...
const App: React.FC = () => {
  ...
  const [currentDifficulty, setCurrentDifficulty] = useState<Difficulty>(Difficulty.HARD);
  ...
  const saveScoreToSupabase = useCallback(async (finalScore: number) => {
    ...
    try {
      await supabase.from('leaderboard').insert([{ username, score: finalScore, difficulty: currentDifficulty }]);
  ...
  }, [username, fetchLeaderboard, dbStatus, currentDifficulty]);
